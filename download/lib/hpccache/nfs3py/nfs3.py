...
class nfs_fh3(vararray_limited):
    '''
    This may be used with both hpccache and non-hpccache filehandles, so the default __str__())
    does not decode it as an hpccache filehandle. In the future, we may want to add
...
class exportHpcCache(ONCRPCwire):
...
class MOUNT3_EXPORTres(OkayOrFailRPCres):
    ...
    def __init__(self, status=None, **kwargs):
        ...
        super().__init__(status=status, **kwargs)
        self.exports = None # list(exportHpcCache)
    ...
    def read(cls, stream):
        ...
        '''
        ret = cls()
        ret.exports = cls.read_list(stream, exportHpcCache.read)
        return ret
