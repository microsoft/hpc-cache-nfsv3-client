...
import nfs3py.nfs3 as nfs3

class LoggerState(nfs3.LoggerState):
    LOGGER_NAME_DEFAULT = 'nfs3_util'
...
class MOUNT3_Client(nfs3.MOUNT3_Client):
    ...
    def find_root_in_export_list(self, logger=None):
        ...
        if res.status != nfs3.mountstat3_MNT3_OK:
            ...
            return False
        ret = False
        for exportnode in res.exports:
            logger.debug("export %s", exportnode)
            dirpath = exportnode.ex_dir.to_string()
            if dirpath == "/":
